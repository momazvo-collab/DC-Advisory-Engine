...
function reducer(state: any, action: any) {
  switch (action.type) {
    case "SET_LOCATION_BASE":
      return {
        location: { base: action.payload, emirate: "", country: "" },
        scope: null,
        region: null,
        activity: null,
      };
    case "SET_LOCATION_EMIRATE":
      return {
        ...state,
        location: { ...state.location, emirate: action.payload },
      };
...
export default function App() {
  ...
  const totalSteps =
    formState.scope === "International" ? 5 : 4;

  const stepLabels =
    formState.scope === "International"
    ...
    <div className="min-h-screen bg-[#F4F7FA] font-sans">
      <div className="bg-[#003B5C] text-white px-6 sm:px-10 lg:px-12 py-8 shadow-md sticky top-0 z-50">
        ...
        <p className="text-sm opacity-80 mt-2">
          Identify the right services based on your business profile
        </p>
      </div>
          ...
          <CardContent className="p-6 sm:p-8 lg:p-12">
            ...
            {step === 1 && (
              <StepLocation
                ...
                onSetBase={(val: any) =>
                  dispatch({ type: "SET_LOCATION_BASE", payload: val })
                }
                onSetEmirate={(val: any) =>
                  dispatch({ type: "SET_LOCATION_EMIRATE", payload: val })
                }
                onSetCountry={(val: any) =>
                  dispatch({ type: "SET_LOCATION_COUNTRY", payload: val })
                }
            ...
            {step === 2 && (
              <StepScope
                ...
                onSelect={(val: any) =>
                  dispatch({ type: "SET_SCOPE", payload: val })
                }
                onNext={() => setStep((p) => p + 1)}
            ...
            {(
              ...
              <StepActivity
                ...
                onChange={(activityObj: any) =>
                  dispatch({ type: "SET_ACTIVITY", payload: activityObj })
                }
                onNext={() => setStep((p) => p + 1)}
                onBack={() => setStep((p) => p - 1)}
              />
